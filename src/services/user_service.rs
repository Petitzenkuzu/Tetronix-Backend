...
impl<T: UserRepositoryTrait> UserServiceTrait for UserService<T> {
    ...
    async fn create(&self, name: &str) -> Result<(), ServicesError> {
        if name.len() < 2 {
            return Err(ServicesError::InvalidInput {
                field: "name".into(),
                message: "must be at least 2 characters long".to_string(),
            });
        }
        if name.len() > 50 {
            return Err(ServicesError::InvalidInput {
                field: "name".into(),
                message: "must be less than 50 characters long".to_string(),
            });
        }
        ...
        match result {
            ...
            Err(e) => match e {
                RepositoryError::AlreadyExists { what } => {
                    Err(ServicesError::AlreadyExists { what })
                }
    ...
    async fn get_top(&self, limit: i32) -> Result<Vec<User>, ServicesError> {
        ...
        match result {
            Ok(users) => Ok(users),
            Err(e) => match e {
                RepositoryError::InvalidLimit { low, high } => Err(ServicesError::InvalidInput {
                    field: "limit".into(),
                    message: format!(
                        "Limit must be greater or equal to {} and less or equal to {}",
                        low, high
                    ),
                }),
                _ => Err(ServicesError::InternalServerError(e.to_string())),
